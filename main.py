...
async def 주제추가(interaction: discord.Interaction, 주제: str):
    if 주제 in data:
        await interaction.response.send_message("이미 있는 주제야", ephemeral=True)
    else:
        data[주제] = []
...
async def 주제보기(interaction: discord.Interaction):
    if not data:
        await interaction.response.send_message("주제가 없어")
    else:
...
async def 목록보기(interaction: discord.Interaction, 주제: str):
    if 주제 not in data or not data[주제]:
        await interaction.response.send_message("목록이 없어")
    else:
...
async def 추첨(interaction: discord.Interaction, 주제: str):
    if 주제 not in data or not data[주제]:
        await interaction.response.send_message("뽑을 게 없어")
    else:
